...
public class MainFrame extends JFrame {
    ...
    private int  currentStepLine = 0;  // 0-based index into editor lines
    ...
    private JPanel buildHeader() {
        ...
        JLabel hint = new JLabel(
            "create A │ newarray arr int 3 │ astore arr 0 A │ link A B │ push main arr A │ pop │ gc    "
        );
    ...
    private void runAll(JLabel lineLabel) {
        reset(lineLabel);
        List<String> lines = editorPanel.getLines();
        logPanel.log("info", "▶ Running " + lines.size() + " line(s)...");
        editorPanel.clearHighlight();

        for (int i = 0; i < lines.size(); i++) {
            final int idx = i;
            editorPanel.setCurrentLine(idx);
            execLine(lines.get(i), idx + 1);
        }

        editorPanel.clearHighlight();
        statsPanel.setStatus("✔ Execution complete");
    ...
    private void reset(JLabel lineLabel) {
        ...
        if (lineLabel != null) lineLabel.setText("Line: —");
        refreshAll();
    }
